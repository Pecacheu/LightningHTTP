...
void SSLList::free() { for(i=0; i<len; i++) SSL_CTX_free((SSL_CTX *)s[i]); }
...
name(*new string(s.name+"->"+c.addr.host+":"+to_string(c.addr.port)+" ("+to_string(c.sck)+")")) {}

HttpSocket::HttpSocket(Socket c):srv(0),cli(c),
name(*new string(string(c.addr.host)+":"+to_string(c.addr.port)+" ("+to_string(c.sck)+")")) {}
...
bool HttpSocket::initCli(bool https, HttpResFunc& cb) {
	if(https) {
		...
		if(SSL_connect(s) <= 0) { error("SSL "+name,-1); cb(-5,0,0); cclose(); return 0; }
	}
	thread([this,cb]() {
		char b[HTTP_READ_SIZE]; uint8_t r;
		while(!(r=run(b)));
		if(r==1) cb(0,req,0); else if(r==2) {
			uint16_t s = eRes->getStat();
			cb(s,0,new string(to_string(s)+" "+eRes->getStatMsg()));
		} else cb(-6,0,0); cclose();
	}).detach(); return 1;
}

uint8_t HttpSocket::run(char *b) {
	...
	#endif
	uint8_t r = parse(Buffer(b,len));
	#if HTTP_DEBUG
	cout << name+" "+(req?req->type+" "+req->uri:(cBuf?to_string(cBuf->len)+" ":"")+"Read More")+"\n";
	#endif
	return r;
}

uint8_t HttpSocket::parse(Buffer r) {
	if(cBuf) { //Read More Data:
		if(chk) return parseChunk(r.buf,r.len);
		bool d=bCopy(r.buf,r.len); if(d) cBuf=0; return d;
	}
	...
	while((i=bFind(r,HTTP_NEWLINE,o)) != string::npos) {
		...
		if(ph) { //Parse Head:
			...
			if(hk == "Content-Length") {
				cl=strToUint(hv); if(chk || cl == string::npos) { sendCode(400, "Invalid Content-Len"); return 2; }
	...
	cOfs=0, cBuf=new Buffer(cl), req=new HttpRequest(*this,t,u,hd,cd,*cBuf);
	if(chk) return parseChunk(r.buf+o,r.len-o);
	if(cl) { bool d=bCopy(r.buf+o,r.len-o); if(d) cBuf=0; return d; }
	cBuf=0; return 1;
}

bool HttpSocket::bCopy(const char *buf, size_t len, size_t *rs) {
	size_t bl=cBuf->len, rl=min(bl-cOfs,len); memcpy((void*)(cBuf->buf+cOfs),buf,rl);
	if(rs) *rs=rl; bool r = (cOfs += rl) == bl; if(r) cOfs=0; return r;
}
uint8_t HttpSocket::parseChunk(const char *buf, size_t len) {
	...
	for(i=0; (p[i] != ';' && p[i] != '\r'); i++) {
		if(i==cd) { cBuf=0; sendCode(400, "Invalid Chunk"); return 2; }
		ck[i] = p[i];
	}
	ck[i]=0; string s=ck; cd=hexStrToUint(s); Buffer b=Buffer(buf,len); i=bFind(b,"\n",i);
	if(cd == string::npos || i == string::npos) { cBuf=0; sendCode(400, "Invalid Chunk-Len"); return 2; }
	if(!cd) { req->content=*cBuf; cBuf=0; return 1; }
	buf+=i+1; len-=i+1; cOfs = cBuf->len; cBuf = new Buffer(Append::buf(*cBuf,Buffer(cd)));
...
inline ssize_t HttpSocket::write(Buffer b) {
	#if HTTP_DEBUG > 2
	char *cs = (char*)strCpy(b.buf,b.len);
...
HttpResponse::HttpResponse(HttpSocket& c, bool k):cli(c),keepAlive(k),cm(0),cont(new Buffer()) {}
HttpResponse::HttpResponse(HttpSocket& c):cli(c),keepAlive(0),cm("GET /"),cont(new Buffer()) {}

bool HttpResponse::sendCode(uint16_t code, string msg, string desc) {
	if(stat) return 0; keepAlive=0,useChunked=0; if(!writeHead(code,msg,0)) return 0; string m = to_string(code)+" "+msg;
...
bool HttpResponse::writeHead(uint16_t code, string status, stringmap *headers) { //Server Mode
	if(cm || ended || code < 100 || code > 999 || (useChunked && stat)) return 0;
	if(useChunked && (code < 200 || code == 204 || code >= 300 && code < 400)) return 0;
	stat=code, sMsg=status, hdr=headers;
	if(useChunked && cli.write(genHeader()) <= 0) return 0;
	return 1;
}

const char *HRPath="/", *HRMet="GET";
bool HttpResponse::writeHead(const char *path, const char *method, stringmap *headers) { //Client Mode
	if(!cm || ended || (useChunked && stat)) return 0;
	stat=1; if(!path) path=HRPath; if(!method) method=HRMet;
...
bool HttpResponse::write(Buffer b) {
	if(ended || b.len < 1) return 0;
	if(useChunked) { if(!stat || cli.write(genChunk(b)) <= 0) return 0; }
	else cont = new Buffer(Append::buf(*cont,b)); return 1;
}
...
Buffer HttpResponse::genHeader() {
	...
	if(useChunked) hd["Transfer-Encoding"] = "chunked";
	...
	if(sc != hd.end()) {
		Buffer b = sc->second; vector<Buffer> cl = bSplit(b,"&"); hd.erase("Set-Cookie");
		for(Buffer& c: cl) h += "Set-Cookie: "+c.toStr()+HTTP_NEWLINE;
	}

	for(auto& kv: hd) h += kv.first+": "+kv.second+HTTP_NEWLINE;
	if(cm) return Append::buf(cm," HTTP/1.1",(sMsg.size()?" "+sMsg:""),HTTP_NEWLINE,h,HTTP_NEWLINE);
	return Append::buf("HTTP/1.1 ",stat,(sMsg.size()?" "+sMsg:""),HTTP_NEWLINE,h,HTTP_NEWLINE);
}

bool HttpResponse::end() {
	if(ended) return 0; if(!stat && !cm) writeHead(useChunked||!cont->len?204:200);
	bool s = cli.write(useChunked?genChunk(*new Buffer()):Append::buf(genHeader(),*cont)) <= 0;
	ended = 1; if(!keepAlive && !cm) cli.cclose(); return s;
}
