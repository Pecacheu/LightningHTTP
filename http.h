...
struct HttpRequest {
	...
	HttpSocket& cli; string type,uri,path,query; stringmap header; Buffer content; uint16_t code; void *u;
};

class HttpSocket {
	void *ssl=0; Buffer *cBuf=0; size_t cOfs;
	...
	inline void cclose(); inline ssize_t write(Buffer b);
	private: uint8_t run(char *b); uint8_t parse(Buffer b);
...
class HttpResponse {
	...
	inline bool writeHead(uint16_t code, stringmap *headers=0) { return writeHead(code,"",headers); }
